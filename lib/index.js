...
*/
const SWITCH_MODE_DESCRIPTION = "切换工作方法论，用于任务性质变化时：从「只想清楚」切到「动手改」、从「摸结构」切到「下结论」。五模式：planner 方案设计与任务拆解、analyst 影响评估与问题定位、explorer 只读代码探索、executor 执行既定方案、base 默认姿态。方法论随本次工具结果注入，引导本阶段的工作方式——是引导不是清单，按情况取舍，不必逐条对照。不是每个任务都要切：简单问答直接答，默认姿态足够；一次切换只调一次，同阶段不重复切。切有打断成本：阶段没变、打法没变就不切，别为切而切。重复切换同名模式可重读方法论（幂等——上下文被压缩后需要时用）。派发分析/执行类 subagent 时，要求对方先 switch_mode 到对应模式再动手。";
/** Shared values that hold in every mode (defaults first, protocol as the exception). */
const CORE_BEHAVIOR = `## 工作准则
...
*/
function foldMode(events, end = events.length) {
	let mode;
	let index = 0;
...
*/
function effectiveMode(events) {
	return foldMode(events) ?? "base";
}
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		execute(args, exec) {
			...
			if (agent === void 0) throw new Error(`${SWITCH_MODE} requires a calling agent (no session to record the mode)`);
			agent.session.append("mycel/mode", { mode: args.mode });
			return Promise.resolve({
				mode: args.mode,
				mindset: MODES[args.mode].mindset
			});
		},
		presentCall: (args) => ({
			card: "generic",
			title: `Switch mode → ${args.mode}`,
			kind: "other"
		})
	}));
}
/** Cordis function-plugin name. */
const name = "mycel-mode";
/** Services required before the plugin can register its section and tool. */
const inject = ["tools", "systemPrompt"];
//#endregion
export { CORE_BEHAVIOR, DEFAULT_MODE, MODES, MODE_NAMES, SWITCH_MODE, SWITCH_MODE_DESCRIPTION, apply, effectiveMode, foldMode, inject, name };
